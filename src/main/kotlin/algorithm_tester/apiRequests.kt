 ...
 */
fun getMonthPGNList(monthlyArchiveUrl: String): List<PGN> {
    val json = getJsonResponse(monthlyArchiveUrl)

    if (json.json == "{\"code\":0,\"message\":\"Data provider not found for key \\\"${
            monthlyArchiveUrl.substringAfter(CHESS_COM_API_URL)
        }\\\".\"}"
    ) {
        throw Exception("Monthly archive does not exist.")
    }
